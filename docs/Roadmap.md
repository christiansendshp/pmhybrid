...
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | ------ | ---------- |
| GAP-28 | Dual-format read+write for `Roadmap.md`/`Agentslog.md` (old table/4-bullet and the project-documentation skill v2 YAML-per-entry schema, installed 2026-09-17) landed already — `roadmap-parser.service.ts`, `roadmap-yaml-entry.util.ts`, `roadmap-row-writer.util.ts`, `agentslog-parser.service.ts`/`agentslog-writer.util.ts` all branch on format internally and are regression-tested (their `.spec.ts` files). One parity bug found in this same pass and fixed: a new-format entry with `status: BLOCKED` lost that status on any lifecycle write-back, unlike the old format's Blocked table which has no Status column to overwrite | Remaining: convert this repo's own `docs/Roadmap.md` to the new schema once it is safe to — deliberately not done in this pass, since the running dev server syncs this repo's own docs every 5 minutes and a botched conversion would corrupt the file this session's audit trail lives in. Note for whoever does it: `roadmapYamlEntryToRow` maps every non-blocked entry to `RoadmapTable.ACTIVE` (the new schema has no ACTIVE/NEAR_TERM split), so the conversion moves every current Near-term row to Active and emits one `ROADMAP_TABLE_CHANGE` audit event per row — expected, call it out in that commit's message. Features/log cross-reference retrofit stays tracked separately, out of scope here | EN_DESARROLLO | Claude | —          |
