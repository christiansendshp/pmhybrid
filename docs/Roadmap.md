...
next_action: >
  ...
  matching a literal "## Cross-cutting" heading line. Correction (this
  entry previously overstated its own blocker): TECH_DEBT-01 is also in
  this file, so closing BUG-02 alone does not empty docs/Roadmap.md --
  there is no ordering constraint on closing BUG-02 today. The real
  constraint is narrower and applies later: whoever removes the *last*
  remaining entry from this file must land the detector fix (and its
  empty-fixture regression test) in that same change, not as a separate
  follow-up, or the mass-sweep hazard this entry describes re-arms the
  moment the file is actually empty. Also scope the fix itself precisely:
  making looksLikeNewFormatRoadmap recognize an empty new-format file only
  changes which code path produces an empty parse -- both the old-format
  and new-format paths already return the same `[]` today, so this fix
  does not by itself close the reconcileRoadmap mass-sweep risk; that
  risk is a separate, product-behavior-scoped question (should an empty
  Roadmap.md ever silently complete every tracked Task?) worth its own
  DECISION entry if it's ever hit for real, not assumed solved here.
