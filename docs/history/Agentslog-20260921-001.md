...
## [2026-09-21T20:51:17Z] | claude | GAP-38 | IN_PROGRESS
- Summary: Read the phase and epic headings of the latest skill's Plan tables as Phase and Epic records and place the rows under them
- Verify: pending

## [2026-09-21T20:57:19Z] | claude | GAP-38 | DONE
- Summary: A project in the latest skill's table format now has phases and epics: inside ## Plan a ### heading of the shape ID - Title is a Phase and a #### heading an Epic (identified by the id in the heading, reusing GAP-35d's externalId and hierarchy sync), the rows of the tables under them are placed in that epic and phase, and a Gaps row in the phase its Phase column names; a row outside the Plan is placed by nothing (ids are never parsed). Headings of another shape, outside the Plan or in a code fence name nothing. The tables have no column to write a move into, so a move in PM Hub is not written, and holds only for a task under no heading. /progress reports the phases. Autonomous decisions, reversible.
- Files: apps/api/src/modules/roadmap/markdown-table.util.ts,apps/api/src/modules/roadmap/roadmap-parser.service.ts,apps/api/src/modules/synchronization/hierarchy-sync.service.ts,apps/api/src/modules/synchronization/synchronization.service.ts,apps/api/test/roadmap-skill-hierarchy.e2e-spec.ts,docs/roadmap-parser.md,docs/synchronization.md
- Verify: api unit 447 and e2e 299 pass (coverage 88.0); web unit 241 and eslint clean; api lint and nest build ok
